import { baseConfig, envData } from './project-config.ts';
import * as fs from 'fs';

export class DeviceCapabilities {
  private static jsonFilePath = "./devices"
  ...
  private static getFilePath(deviceType: string, isRemote: boolean): string {
    ...
    if (baseConfig.isApiTest) {
      return `${DeviceCapabilities.jsonFilePath}/api/api.json`; // As WDIO does not support API testing, we are using api.json for API testing in headless mode
    }
  ...
  private static configureMobileCapabilities(capabilities: JSON[]): JSON[] {
    ...
    mobileCapabilities.map((capability: any) => {
      if(baseConfig.isMobileApp) {
        if(appDetails.path) {
          capability['appium:app'] = appDetails.path;
        }
        if(appDetails.appPackage && baseConfig.device === 'android') {
          capability['appium:appPackage'] = appDetails.appPackage;
          capability['appium:appActivity'] = appDetails.appActivity;
        }
      }
    });
...
// Read from .env file or use default values
export const appDetails = {
  path: envData.APP_PATH || undefined,
  appPackage: envData.APP_PACKAGE || undefined,
  appActivity: envData.APP_ACTIVITY || undefined
}